...
class inland_fim():
    def __init__(self):
        super(inland_fim, self).__init__()
        
    def inundate(self,rem, catchments, catchment_poly, hydro_table, forecast,
        ...
        if hydro_table is not None:
            catchmentStagesDict,hucSet = self.__subset_hydroTable_to_forecast(huc_6_id,hydro_table,forecast,subset_hucs)
        else:
        ...
        if catchmentStagesDict is not None:
            if src_table is not None:
                self.create_src_subset_csv(hydro_table,catchmentStagesDict,src_table)

            # make windows generator
            window_gen = self.__make_windows_generator(rem, catchments, catchment_poly,
            ...
            # submit jobs
            results = {executor.submit(self.__inundate_in_huc,*wg) : wg[6] for wg in window_gen}
    ...
    def __inundate_in_huc(self,rem_array,catchments_array,crs,window_transform,rem_profile,catchments_profile,hucCode,
        ...
        if hucCode is not None:
            self.__vprint("Inundating {} ...".format(hucCode),not quiet)
        ...
        if isinstance(depths,str):
            depths = self.__append_huc_code_to_file_name(depths,hucCode)
        ...
        if isinstance(inundation_raster,str):
            inundation_raster = self.__append_huc_code_to_file_name(inundation_raster,hucCode)
        ...
        if inundation_polygon is not None:
            ...
            if isinstance(inundation_polygon,str):
                inundation_polygon = self.__append_huc_code_to_file_name(inundation_polygon,hucCode)
        ...
        # make output arrays
        inundation_array,depths_array = self.__go_fast_mapping(rem_array,catchments_array,catchmentStagesDict,inundation_array,depths_array)
    ...
    @njit
    def __go_fast_mapping(self,rem,catchments,catchmentStagesDict,inundation,depths):
    ...
    def __make_windows_generator(self,rem, 
                                 catchments,
                                 catchment_poly,
                                 mask_type,
                                 catchmentStagesDict,
    ...
    def __append_huc_code_to_file_name(self,fileName,hucCode):
    ...
    def __subset_hydroTable_to_forecast(self,huc_6_id, hydroTable,forecast,subset_hucs=None):
        ...
        if isinstance(forecast,str):
            ...
            try:
                ...
                forecast = self.read_nwm_forecast_file(forecast)
    ...
    def read_nwm_forecast_file(self,forecast_file,rename_headers=True):
    ...
    def __vprint(self, message,verbose):
        if verbose:
            print(message)

    def create_src_subset_csv(self,hydro_table,catchmentStagesDict,src_table):
